...
void ZigbeeCoapResource::handler_put(CoAPCallback &callback)
{
    ...
    {
        ACE_DEBUG((LM_DEBUG, "server recv payload: %s\n", payload.c_str()));
...
void ZigbeeCoapResource::do_on_off_cmd(unsigned char id)
{
    ...
    if (zigbee_node_)
    {
        ...
        ZigbeeCommand * c =
        ZigbeeCommand::create_AF_DATA_REQ_cmd(zigbee_node_->get_short_addr(),
                                                        zigbee_ep_,
