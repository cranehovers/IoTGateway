...
handle_request(coap_context_t *context, coap_queue_t *node)
{
    ...
    if (!resource)
    {
        ...
        case COAP_REQUEST_GET:
            ...
            else     /* GET request for any another resource, return 4.04 */
            {
                ...
                info("GET for unknown resource 0x%02x%02x%02x%02x, return 4.04\n",
        ...
        default: 			/* any other request type */
            ...
            info("unhandled request for unknown resource 0x%02x%02x%02x%02x\r\n",
                  key[0], key[1], key[2], key[3]);
    ...
    if (h)
    {
        info("call custom handler for resource 0x%02x%02x%02x%02x\n",
              key[0], key[1], key[2], key[3]);
                           ...
                           node->pdu->hdr->token))
        {
                     ...
                     && !coap_is_mcast(&node->local)))
            {
                ...
                {
                    info("cannot send response for message %d\n", node->pdu->hdr->id);
                }
            }
    ...
    else
    {
        ...
        if (!response || (coap_send(context, &node->remote, response)
                          == COAP_INVALID_TID))
        {
            info("cannot send response for transaction %u\n", node->id);
        }
        coap_delete_pdu(response);
    }
}
